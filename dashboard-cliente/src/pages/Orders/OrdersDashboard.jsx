...
import { User, MapPin, Map, AlertCircle, XCircle, Printer, Edit, Receipt, Lock, RefreshCw, Trash2, Eye, Inbox, Save, Check } from 'lucide-react';
    ...
    return (
      <div key={order.id} style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px dashed #f1f5f9', paddingBottom: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#64748b', fontSize: '0.9rem', fontWeight: 600 }}><User size={14} /> {order.waiterName || 'Sistema'}</div>
        ...
        {branches.length > 0 && order.branchId && (
          <div style={{ marginBottom: '0.75rem', fontSize: '0.75rem', color: '#6366f1', background: '#eef2ff', display: 'inline-block', padding: '2px 8px', borderRadius: '4px', fontWeight: 'bold' }}>
            <MapPin size={10} style={{ marginRight: '2px', display: 'inline-block', verticalAlign: 'middle' }} /> {branches.find(b => b.id === order.branchId)?.name || 'Sede'}
          </div>
        )}
          ...
          {order.items?.map((item, idx) => (
            <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '0.5rem', fontSize: '0.95rem' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {item.sku && (
                  <span style={{ fontSize: '0.75rem', color: '#64748b', display: 'block', fontWeight: 600, marginTop: '2px' }}>SKU: {item.sku}</span>
                )}
        ...
        {order.returnedItems && order.returnedItems.length > 0 && (
          <div style={{ margin: '1rem 0', padding: '0.75rem', background: '#fff1f2', borderRadius: '12px', border: '1px solid #fecaca' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 800, color: '#b91c1c', marginBottom: '0.5rem', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '4px' }}><RefreshCw size={12} /> Productos Devueltos</div>
        ...
        {order.globalObservations && (
          <div style={{ fontSize: '0.9rem', color: '#92400e', marginBottom: '1rem', padding: '1rem', background: '#fef3c7', borderRadius: '12px', borderLeft: '4px solid #f59e0b', fontStyle: (order.globalObservations.includes('(Comandado por') || order.globalObservations.includes('(Vendido por')) ? 'italic' : 'normal' }}>
            {order.globalObservations}
          </div>
        )}
        ...
        <div style={{ marginTop: '1.25rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap', borderTop: '1px solid #f1f5f9', paddingTop: '1rem', justifyContent: 'flex-end' }}>
          ...
          {(order.status === 'pending' || order.status === 'preparing') && (
            <button onClick={() => handleOpenEdit(order)} style={{ background: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '0.5rem 0.85rem', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>Editar</button>
          )}
          ...
          {order.status === 'preparing' && (
            <>
              <button className="btn-primary" style={{ flex: 1, padding: '0.5rem', fontSize: '0.875rem', backgroundColor: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }} onClick={() => startVerification(order, 'dispatched')}><Check size={14} /> Despachado</button>
              ...
              {order.isBilled && <button style={{ background: '#1e293b', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.5rem', fontSize: '0.875rem', cursor: 'pointer', fontWeight: 700 }} onClick={() => printTicket(order, RESTAURANT_NAME, 'invoice')}>Imprimir Factura</button>}
      ...
      <div className="orders-dashboard-page">
        ...
        <div style={{ background: '#f8fafc', padding: '4rem 2rem', borderRadius: '24px', textAlign: 'center', marginTop: '2rem' }}>
          ...
          <p style={{ color: '#64748b', maxWidth: '400px', margin: '0 auto 1.5rem', lineHeight: '1.6' }}>
            ...
            Para recibir pedidos y gestionar comandas aquí, debes mejorar el plan a <strong>Plan Pro</strong> desde tu suscripción.
          </p>
    ...
    <div className="orders-dashboard-page">
      <header className="orders-header" style={{ marginBottom: '2rem' }}>
        <div className="header-title-group">
          <h1 style={{ fontSize: '2rem', fontWeight: 900, color: '#0f172a', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Inbox size={28} /> Bandeja de Entrada
          </h1>
        ...
        <div className="branch-selector-card" style={{ background: 'white', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <label style={{ fontWeight: 700, color: '#475569', display: 'inline-flex', alignItems: 'center', gap: '4px' }}><MapPin size={16} /> Sede:</label>
          <select 
          ...
          <div className="orders-list" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem' }}>
            ...
            {pendingInboxOrders.length === 0 && (
              <div style={{ textAlign: 'center', padding: '5rem 2rem', color: '#94a3b8', background: 'white', borderRadius: '20px', border: '2px dashed #e2e8f0', gridColumn: '1 / -1' }}>
                <div style={{ color: '#94a3b8', display: 'flex', justifyContent: 'center', marginBottom: '1.5rem' }}>
        ...
        <div className="modal-overlay">
           <div className="modal-content edit-order-modal" style={{ maxWidth: '900px', width: '95%', borderRadius: '24px' }}>
             <header className="modal-header" style={{ padding: '1.5rem 2rem', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
               <h2 style={{ fontSize: '1.5rem', fontWeight: 900, display: 'inline-flex', alignItems: 'center', gap: '8px' }}><Edit size={24} /> Editar Comanda #{orderToEdit.id.slice(-6).toUpperCase()}</h2>
                    ...
                    {editForm.items.map((item, idx) => (
                      <div key={idx} style={{ background: '#f8fafc', padding: '1rem', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #e2e8f0' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '0.85rem', color: '#64748b' }}>${(item.price || 0).toLocaleString()} c/u</div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                          ...
                          <button style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center' }} onClick={() => { const newItems = editForm.items.filter((_, i) => i !== idx); setEditForm({ ...editForm, items: newItems }); }}><Trash2 size={16} /></button>
                        </div>
                      </div>
                    ))}
                 </div>
        ...
        <div className="pos-modal-overlay" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, background: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(4px)', padding: '20px' }}>
          <div className="pos-modal-content" style={{ maxWidth: '600px', width: '90%', padding: '2rem', borderRadius: '24px' }}>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
               <div style={{ color: 'var(--primary)', display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}><RefreshCw size={48} strokeWidth={1.5} /></div>
